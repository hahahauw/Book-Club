...
async function selectCatalogBook(index) {
  ...
  const book = await loadCatalogDetails(result); state.catalogBook = book; state.catalogDuplicateConfirmation = ""; ui.catalogGenre.value = book.genre || "";
...
async function saveCatalogShelfBook(book, status) {
  ...
  const metadata = { title: book.title, author: book.author, genre: ui.catalogGenre.value.trim(), coverUrl: book.coverUrl, catalogKey: book.catalogKey, catalogId: book.catalogId, isbn: book.isbn, publicationYear: book.publicationYear, synopsis: book.synopsis, source: book.source, status, note: ui.catalogShelfNote.value.trim(), date: new Date().toISOString() };
...
async function saveCatalogRecommendation(book) {
  ...
  const previousReasons = await getDocs(collection(sharedRef, "recommendations"));
  const otherReaders = previousReasons.docs.map((entry) => entry.data()).filter((entry) => entry.memberId !== state.user.uid).map((entry) => entry.displayName || "a club member");
  if (!exists) await setDoc(sharedRef, { title: book.title, author: book.author, genre: ui.catalogGenre.value.trim(), coverUrl: book.coverUrl, synopsis: book.synopsis || "", catalogKey: book.catalogKey, isbn: book.isbn || "", publicationYear: book.publicationYear || "", source: book.source, date: new Date().toISOString(), comments: [] });
  ...
  const reaction = !exists ? "You're the first to recommend this book — you started something." : otherReaders.length === 1 ? `You and ${otherReaders[0]} both recommend this one.` : otherReaders.length > 1 ? `You, ${otherReaders[0]}, and ${otherReaders.length - 1} other member${otherReaders.length === 2 ? "" : "s"} recommend this one.` : "Your recommendation just joined the club shelf.";
...
async function addShelfBook(event) { event.preventDefault(); if (!isMember() || !state.openProfileId) return; try { const file = $("shelfFile")?.files?.[0]; const coverUrl = $("shelfCover").value.trim() || (file ? await uploadImage(file) : ""); await addDoc(collection(db, "memberShelves", state.openProfileId, "entries"), { title: $("shelfTitle").value.trim(), author: $("shelfAuthor").value.trim(), coverUrl, status: $("shelfStatus").value, note: $("shelfNote").value.trim(), date: new Date().toISOString() }); event.target.reset(); toast("Added to your personal shelf — your library is growing."); } catch (error) { console.error(error); toast(error.message || "Could not add that book."); } }
